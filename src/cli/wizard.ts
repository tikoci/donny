...
type WizardOperation = "info" | "devices" | "export" | "add";

function isReadonlyWizardOperation(op: WizardOperation): boolean {
  return op !== "add";
}

export async function runWizard(initialDbPath?: string): Promise<void> {
  ...
  const dbPath = initialDbPath ?? await clack.text({
  ...
  let db: DudeDB;
  try {
    db = DudeDB.open(dbPathText, { readonly: isReadonlyWizardOperation(op) });
  } catch (err) {
    clack.log.error(`Cannot open ${dbPathText}: ${err instanceof Error ? err.message : String(err)}`);
    process.exit(1);
  }
  ...
  try {
    switch (op) {
      case "info": {
        ...
        clack.log.info(`Database statistics for ${bold(dbPathText)}:`);
        printStats(stats);
        break;
      }
