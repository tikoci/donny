#!/usr/bin/env bun
/**
 * donny CLI — command router for dude.db operations.
 *
 * Usage:
 *   donny                        interactive wizard (TTY only)
 *   donny setup                  interactive wizard (explicit)
 *   donny info <db>              show database statistics
 *   donny list devices <db>      list all devices
 *   donny list probes <db>       list all probe templates
 *   donny export <db>            export devices (--format csv|json, --include-credentials)
 *   donny add device <db>        add a device (--name, --address, --username, --password)
 */
...
// Auto-wizard on plain TTY with no args
...
const { flags, positional } = parseFlags(args.slice(command === "list" || command === "add" ? 2 : 1));
